...
function TareasP() {
    ...
    const obtenerTareas = async () => {
        try {
            const resultado = await fetch('http://localhost:4000/tareas?Estatus=Actividades')
            if (!resultado.ok) {
    ...
    <div>
        ...
        {showForm && (
            <div className='mx-12 mt-5'>
                <form>
                    <div className="mb-4">
                        <label htmlFor="Descripcion" className="block text-2xl font-semibold text-gray-600">Titulo (máximo 20 caracteres):</label>
                        <input
                            type="text"
                            id="Descripcion"
                            name="Descripcion"
